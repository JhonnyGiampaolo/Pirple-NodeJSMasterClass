...
handlers.exampleError = function(data,callback){
    var err = new Error('This is an example');
    throw (err);
};
            ...
            _data.read('users',phone,function(err,userData){
                if(!err && userData){
                    ...
                    _data.update('users',phone,userData,function(err){
                        if(!err){
                            callback(200,{'Message':'User updated sucessfully'});
                        } else {
                            console.log(err);
                    ...
                    if(!err && userData){
                        ...
                        _data.delete('users',phone,function(err){
                            if(!err){
                                    ...
                                    userChecks.forEach(checkId => {
                                        ...
                                        _data.delete('checks',checkId,function(err){
                                            ...
                                            if(checksDeleted == checksToDelete){
                                                if(!deletionErrors){
                                                    callback(200,{'Message':'User deleted sucessfully'});
                                                } else {
                                ...
                                callback(500,{'Error':'Colud not delete the specified user'})
                            }
                        }); 
                    } else {
...
handlers._tokens.post = function(data, callback){
    ...
    if (phone && password){
        ...
        _data.read('users',phone,function(err,userData){
            if(!err && userData){
                // Hash tje sent password and compare it to the password store in the user object
                var hashedPassword = helpers.hash(password);
            ...
            if (!err && tokenData){
                ...
                if(tokenData.expires > Date.now()){
                    ...
                    _data.update('tokens',id,tokenData,function(err){
                        if (!err){
                            callback(200,{'Message':'The token has been extended for another hour'})
                        } else {
            ...
            if(!err && tokenData){
                ...
                _data.delete('tokens',id,function(err){
                    if(!err){
                        ...
                        callback(500,{'Error':'Colud not delete the specified token'})
                    }
                }); 
            } else {
...
// Verify if a given id is curently valid for a given user
        ...
        _data.read('tokens',token,function(err,tokenData){
            if(!err && tokenData){
                ...
                _data.read('users',userPhone,function(err,userData){
                    if(!err && userData){
                        var userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
                        // Verufy that the user has less than the number of max-checks-per-user
                    ...
                    handlers._tokens.verifyToken(token,checkData.userPhone,function(tokenIsValid){
                        if (tokenIsValid){
                            ...
                            _data.update('checks',id,checkData,function(err){
                                if(!err){
                                    callback(200,{'Message':'Check updated sucessfully'});
                                } else {
                                    console.log(err);
    ...
    if (id){
        ...
        _data.read('checks',id,function(err,checkData){
            if (!err && checkData){
                ...
                //Verify that the token given is valid for the phone number
                handlers._tokens.verifyToken(token,checkData.userPhone,function(tokenIsValid){
                    if (tokenIsValid){
